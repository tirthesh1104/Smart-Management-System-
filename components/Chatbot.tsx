...
const Chatbot: React.FC<ChatbotProps> = ({ context, userRole, actions }) => {
  ...
  const getFallbackResponse = useCallback((text: string): string => {
    const q = text.toLowerCase().trim();
    if (q.includes('attendance') || q.includes('present') || q.includes('absent')) {
      if (selectedLang === 'hi-IN') {
        return "आपकी उपस्थिति दर वर्तमान में 100% है। संपूर्ण उपस्थिति रिकॉर्ड 'Full Attendance Record' टैब में उपलब्ध है।";
      } else if (selectedLang === 'mr-IN') {
        return "तुमची उपस्थिती सध्या 100% आहे. संपूर्ण नोंद 'Full Attendance Record' टॅबमध्ये उपलब्ध आहे.";
      }
      return "Your overall attendance rate is currently 100.0%. You can view subject-wise details under the 'Full Attendance Record' tab.";
    }

    if (q.includes('exam') || q.includes('test') || q.includes('quiz') || q.includes('schedule')) {
      if (selectedLang === 'hi-IN') {
        return "आपकी आगामी परीक्षाएँ 'Exam Portal' टैब में सूचीबद्ध हैं। आप वहाँ से सीधे ऑनलाइन परीक्षा दे सकते हैं।";
      } else if (selectedLang === 'mr-IN') {
        return "तुमच्या आगामी परीक्षा 'Exam Portal' टॅबमध्ये आहेत. तुम्ही तिथून परीक्षा देऊ शकता.";
      }
      return "Upcoming exams (such as Data Structures Mid-Term and Algorithms Quiz 1) are listed under the 'Exam Portal' tab.";
    }

    if (q.includes('leave') || q.includes('holiday') || q.includes('permission')) {
      if (selectedLang === 'hi-IN') {
        return "छुट्टी का आवेदन जमा करने के लिए 'Leave Applications' टैब पर जाएं और '+ Apply for New Leave' पर क्लिक करें।";
      } else if (selectedLang === 'mr-IN') {
        return "रजेचा अर्ज करण्यासाठी 'Leave Applications' टॅबवर जा आणि '+ Apply for New Leave' वर क्लिक करा.";
      }
      return "To request leave, go to the 'Leave Applications' tab and click on the '+ Apply for New Leave' button.";
    }

    if (q.includes('event') || q.includes('fest') || q.includes('sports') || q.includes('cultural')) {
      if (selectedLang === 'hi-IN') {
        return "कैंपस के आगामी कार्यक्रम (जैसे Annual Tech Fest, Sports Day) 'Events' टैब में देखे जा सकते हैं।";
      } else if (selectedLang === 'mr-IN') {
        return "कॅम्पस कार्यक्रम (उदा. Annual Tech Fest, Sports Day) 'Events' टॅबमध्ये उपलब्ध आहेत.";
      }
      return "Campus events like Annual Tech Fest 2024, Sports Day, and Cultural Night are available under the 'Events' tab.";
    }

    if (q.includes('class') || q.includes('lecture') || q.includes('live')) {
      if (selectedLang === 'hi-IN') {
        return "अनुसूचित लाइव कक्षाएं और रिकॉर्डिंग 'Live Classes' टैब में पाई जा सकती हैं।";
      } else if (selectedLang === 'mr-IN') {
        return "लाइव्ह वर्ग आणि रेकॉर्डिंग 'Live Classes' टॅबमध्ये पाहू शकता.";
      }
      return "Scheduled live classes and past lecture recordings are accessible in the 'Live Classes' tab.";
    }

    if (q.includes('hi') || q.includes('hello') || q.includes('hey') || q.includes('namaste') || q.includes('help')) {
      if (selectedLang === 'hi-IN') {
        return "नमस्ते! मैं आपका एआई असिस्टेंट हूँ। आप मुझसे उपस्थिति, परीक्षा, लाइव क्लास या छुट्टियों के बारे में पूछ सकते हैं।";
      } else if (selectedLang === 'mr-IN') {
        return "नमस्कार! मी तुमचा AI सहाय्यक आहे. तुम्ही मला उपस्थिती, परीक्षा किंवा वर्गांबद्दल विचारू शकता.";
      }
      return "Hello! 👋 I am your Smart AI Assistant. Ask me anything about your attendance, exams, live classes, leave applications, or campus events!";
    }

    return `I am here to help! Currently viewing: ${context || 'Dashboard'}. You can ask me about your attendance, upcoming exams, live classes, leave applications, or campus events.`;
...
- Keep responses concise.`,
      ...
      console.warn("Gemini chat init fallback:", e);
      chatRef.current = null;
    }

    const initialGreeting = selectedLang === 'hi-IN' 
      ? "नमस्ते! मैं आपका स्मार्ट एआई असिस्टेंट हूँ। मैं आपकी सहायता कैसे कर सकता हूँ?" 
      : selectedLang === 'mr-IN' 
      ? "नमस्कार! मी तुमचा स्मार्ट AI सहाय्यक आहे. मी तुम्हाला कशी मदत करू शकेन?" 
      : "Hello! 👋 I am your Smart AI Assistant. How can I help you today?";
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      console.warn("AI connection fallback:", e);
      const fallbackMessage = getFallbackResponse(text);
      setMessages(prev => [...prev, { sender: 'bot', text: fallbackMessage }]);
      speak(fallbackMessage, selectedLang);
    } finally {
      setIsLoading(false);
    }
  }, [isLoading, actions, speak, selectedLang, getFallbackResponse]);
