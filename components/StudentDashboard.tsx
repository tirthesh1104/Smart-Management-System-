...
interface StudentDashboardProps {
  ...
  onUnregisterFromEvent: (eventId: string) => void;
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ 
  user, onLogout, studentData, onPlanUpdate, onUpdateUser, leaveApplications, onApplyForLeave, 
  exams, examSubmissions, onSubmitExam, liveClasses,
  events, onRegisterForEvent, onUnregisterFromEvent,
  announcements, notifications, onMarkNotificationRead, onMarkAllNotificationsRead, onClearAllNotifications
}) => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'events':
        return <AnimatedElement><EventsView 
            events={events} 
            userId={user.id}
            onRegister={onRegisterForEvent}
            onUnregister={onUnregisterFromEvent}
        /></AnimatedElement>;
      case 'announcements':
        return <AnimatedElement><AnnouncementBoard 
  ...
  const upcomingExamsCount = exams.filter(e => new Date(e.id).getTime() > Date.now()).length;
  const upcomingLiveClasses = liveClasses.filter(c => c.status === 'Scheduled' && new Date(c.scheduledTime).getTime() > Date.now()).length;
          ...
          <AnimatedElement className="mb-6">
            <DashboardStats 
              ...
              stats={{
                ...
                exams: { total: exams.length, upcoming: upcomingExamsCount },
                pendingTasks: studentData.progress.reduce((acc, subject) => acc + subject.assignments.filter(a => a.status === 'Pending').length, 0),
              }}
            />
          </AnimatedElement>
        )}
        ...
        <AnimatedElement className="border-b border-gray-700 mb-6" delay={100}>
          <nav className="-mb-px flex flex-wrap space-x-6" aria-label="Tabs">
            ...
            <button
              ...
              className={`tab-button py-3 px-1 font-medium text-sm text-gray-400 hover:text-white ${activeTab === 'events' ? 'active' : ''}`}
            >
              Events
            </button>
            <button
            ...
            <button
              ...
              className={`tab-button py-3 px-1 font-medium text-sm text-gray-400 hover:text-white ${activeTab === 'announcements' ? 'active' : ''}`}
            >
              Announcements
            </button>
            <button
